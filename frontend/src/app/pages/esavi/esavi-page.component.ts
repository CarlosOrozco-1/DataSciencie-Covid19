/**
 * Página dedicada a análisis ESAVI - Versión Mejorada
...
export class EsaviPageComponent implements OnInit {
  ...
  /**
   * Calcula el gradiente para el gráfico de dona
   */
  getDonutGradient(): string {
    if (!this.resumenEsavi?.por_sexo) return '';
    
    const total = this.resumenEsavi.total_registros;
    const femenino = this.resumenEsavi.por_sexo['Femenino'] || 0;
    
    const femeninoDeg = (femenino / total) * 360;
    
    return `conic-gradient(#FF69B4 0deg ${femeninoDeg}deg, #2563eb ${femeninoDeg}deg 360deg)`;
  }

  /**
   * Calcula la opacidad para la matriz de calor
   */
  getHeatmapOpacity(grupo: string, sexo: string): number {
    if (!this.resumenEsavi) return 0.3;
    
    const totalGrupo = this.resumenEsavi.por_grupo_etario[grupo] || 0;
    const maxGrupo = Math.max(...Object.values(this.resumenEsavi.por_grupo_etario));
    
    // Calcular factor sexo basado en datos reales
    const totalFemenino = this.resumenEsavi.por_sexo['Femenino'] || 0;
    const totalMasculino = this.resumenEsavi.por_sexo['Masculino'] || 0;
    const totalGeneral = totalFemenino + totalMasculino;
    
    const factorSexo = sexo === 'Femenino' 
      ? totalFemenino / totalGeneral 
      : totalMasculino / totalGeneral;
    
    const proporcion = (totalGrupo / maxGrupo) * factorSexo;
    
    return 0.3 + (proporcion * 0.5); // Opacidad entre 0.3 y 0.8
  }

  /**
   * Calcula casos estimados para la matriz de calor
   */
  getEstimatedCases(grupo: string, sexo: string): number {
    if (!this.resumenEsavi) return 0;
    
    const totalGrupo = this.resumenEsavi.por_grupo_etario[grupo] || 0;
    
    // Usar proporciones reales de sexo
    const totalFemenino = this.resumenEsavi.por_sexo['Femenino'] || 0;
    const totalMasculino = this.resumenEsavi.por_sexo['Masculino'] || 0;
    const totalGeneral = totalFemenino + totalMasculino;
    ...
    return Math.round(totalGrupo * factorSexo);
  }
}